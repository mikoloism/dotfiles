...
shagen() {
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      -h|--help|help) print_help; [ "$0" = "$SHELL" ] && exit 0 || return 0; break ;;

      -s|-silent|--no-output)   should_print_output='NO' ; shift        ;;
      -c|--copy|--clipboard)    should_copy_output='YES' ; shift        ;;
      --no-copy|--no-clipboard) should_copy_output='NO'  ; shift        ;;
      -d|--data|data)           data="$2"                ; shift; shift ;;
      -a|--algorithm|algorithm) algorithm="$2"           ; shift; shift ;;

      -*|--*)
        echo "Unknown option $1"
        [ "$0" = "$SHELL" ] && exit 1 || return 1; break;
        ;;
    esac
  done
